  ...
  types: {
    image: ({ value }: any) => {
      ...
      return (
        ...
        >
          <img src={value.url} alt={value.alt || 'article image'} className="w-full h-auto object-cover" />
        </motion.div>
      );
    },
    youtube: ({ value }: any) => {
...
export default function NewsArticleClient({ article, recentArticles }: { article: any, recentArticles: any[] }) {
      ...
      <div className="relative w-full h-[60vh] md:h-[70vh] flex items-end pb-20 pt-32 -mt-16">
        <div className="absolute inset-0 z-0">
          {article.thumbnailUrl ? (
            <img src={article.thumbnailUrl} alt="Hero" className="w-full h-full object-cover opacity-60" />
          ) : (
      ...
      <div className="container-pc !max-w-[1200px] relative pb-32 grid grid-cols-1 lg:grid-cols-[1fr_minmax(auto,720px)_1fr] gap-0 lg:gap-8 px-4">
        ...
        <div className="w-full relative z-10">
          ...
          <div className="mt-32 pt-16 border-t border-white/10">
            <h3 className="text-3xl font-bold text-white mb-10">다른 소식 더 알아보기</h3>
            <div className="flex flex-col gap-12">
              {recentArticles.map(recent => (
                <div key={recent._id} className="group relative">
                  <Link href={`/news/${recent.slug}`} className="absolute inset-0 z-10" aria-label={recent.title} />
              ...
              {recentArticles.length === 0 && (
                <p className="text-gray-500">추천 기사가 없습니다.</p>
              )}
            </div>
          </div>
        </div>
