...
## 2. 작업 폴더 내 필수 전달 파일 구성 (3종)

다른 컴퓨터에서 안티그래비티에게 작업을 요청할 때, 하나의 폴더에 아래 파일들을 모아서 전달합니다:

| 파일 종류 | 역할 및 용도 | 필수 여부 |
| :--- | :--- | :---: |
| **① 노션 내보내기 `.zip`** (또는 압축 푼 폴더) | 글 본문 내용(Markdown/HTML) 및 고화질 이미지 원본 추출 | **필수** |
| **② 노션 순서 확인용 PDF** | 노션 화면 그대로 내보낸 PDF로, 본문과 사진의 정확한 배치 순서 참고용 | **필수** |
| **③ 실제 첨부할 최종 리포트 PDF** | 웹사이트에서 사용자가 '리포트 다운로드' 버튼 클릭 시 다운로드되는 실제 문서 | 선택 (있을 때만) |
